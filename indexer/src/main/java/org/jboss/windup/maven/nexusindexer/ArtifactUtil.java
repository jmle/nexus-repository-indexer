...
public class ArtifactUtil
{
    public static boolean isArtifactAlreadyIndexed(Indexer indexer, IndexingContext context, String sha1, ArtifactInfo artifactInfo) throws IOException
    {
        ...
        final BooleanQuery.Builder hashQueryBuilder = new BooleanQuery.Builder();
        hashQueryBuilder.add(indexer.constructQuery(MAVEN.SHA1, new SourcedSearchExpression(sha1)), BooleanClause.Occur.MUST);
        hashQueryBuilder.add(indexer.constructQuery(MAVEN.GROUP_ID, new SourcedSearchExpression(artifactInfo.getGroupId())), BooleanClause.Occur.MUST);
        hashQueryBuilder.add(indexer.constructQuery(MAVEN.ARTIFACT_ID, new SourcedSearchExpression(artifactInfo.getArtifactId())), BooleanClause.Occur.MUST);
        hashQueryBuilder.add(indexer.constructQuery(MAVEN.VERSION, new SourcedSearchExpression(artifactInfo.getVersion())), BooleanClause.Occur.MUST);
        ...
        hashQueryBuilder.add(indexer.constructQuery(MAVEN.CLASSIFIER, new SourcedSearchExpression(Field.NOT_PRESENT)), BooleanClause.Occur.MUST_NOT);
        BooleanQuery hashQuery = hashQueryBuilder.build();
